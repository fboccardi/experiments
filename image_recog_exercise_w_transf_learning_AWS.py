...
history=model.fit(images_tr_set,labels_tr_set,epochs=epochs, batch_size=batch_size,validation_split=0.1)
...
np.save('training_loss_WTL_'+info_str, loss)
np.save('training_acc_WTL_'+info_str, acc)

np.save('validation_loss_WTL_'+info_str, val_loss)
np.save('validation_acc_WTL_'+info_str, val_acc)

np.save('test_loss_WTL_'+info_str, test_loss)
np.save('test_acc_WTL_'+info_str, test_acc)
